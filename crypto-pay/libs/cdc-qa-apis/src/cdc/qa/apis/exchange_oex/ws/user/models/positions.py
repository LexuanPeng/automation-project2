...
class PositionDetail(FrozenBaseModel):
    account_id: Optional[str] = Field(description="instrument name")
...
class PrivateGetPositionsRequest(ExchangeRequest):
...
class PrivateGetPositionsResponse(ExchangeResponse):
...
class PrivateClosePositionRequest(ExchangeRequest):
...
class PrivateClosePositionResponse(ExchangeResponse):
