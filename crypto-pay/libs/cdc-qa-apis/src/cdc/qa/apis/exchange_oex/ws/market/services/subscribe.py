...
from ..models.mark_instrument import (
...
class MarketSubscribeService(ExchangeWsService):
    def send_book_instrument(
        self,
        instrument_name: str,
        depth: int = None,
        book_subscription_type: str = None,
        book_update_frequency: int = None,
    ):
        channel = f"book.{instrument_name}" if depth is None else f"book.{instrument_name}.{depth}"
        if book_subscription_type is None:
            request = SubscribeBookInstrumentRequest(
                params=SubscribeBookInstrumentRequestParams(channels=[channel])
            ).json(exclude_none=True)
        else:
            request = SubscribeBookInstrumentRequest(
                params=SubscribeBookInstrumentRequestParams(
                    channels=[channel],
    ...
    def get_book_instrument_msgs(
        ...
        **kwargs,
        ...
        subscription = f"book.{instrument_name}" if depth is None else f"book.{instrument_name}.{depth}"
        return list(
            map(
