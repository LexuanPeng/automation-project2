...
class GetPositionsRequestParams(FrozenBaseModel):
    instrument_name: Optional[str] = Field(default=None, description="e.g. BTC_USDT, ETH_CRO, etc.")


class GetPositionsRequestBody(DerivativesSignedRequest):
...
class GetPositionsResponse(DerivativesResponse):
...
class ClosePositionRequestParams(FrozenBaseModel):
...
class ClosePositionRequestBody(DerivativesSignedRequest):
...
class ClosePositionResponse(DerivativesResponse):
